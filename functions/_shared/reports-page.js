...
<link rel="stylesheet" href="/styles.min.css?v=20260914-1"><link rel="stylesheet" href="/reports.css?v=20260914-1"></head>
...
<script src="/clarity-consent.min.js?v=20260914-1" defer></script><script src="/reports.js?v=20260914-1" defer></script></body></html>`;

const image = (item, index) => `<figure><img src="/api/reparaturberichte/bild/${encodeURIComponent(item.id)}" alt="${escapeHtml(item.alt_text)}" loading="${index ? "lazy" : "eager"}" width="1200" height="800"><figcaption>${escapeHtml(item.alt_text)}</figcaption></figure>`;

export function renderIndex(reports) {
  const cards = reports.length ? reports.map((report) => `<article class="report-card"><a href="/reparaturberichte/${encodeURIComponent(report.slug)}">${report.image_id ? `<img src="/api/reparaturberichte/bild/${encodeURIComponent(report.image_id)}" alt="${escapeHtml(report.image_alt || "Reparaturfoto")}" loading="lazy" width="720" height="480">` : `<div class="report-placeholder" aria-hidden="true">MK</div>`}<div class="report-card-copy"><span class="report-category">${escapeHtml(report.category)}</span><h2>${escapeHtml(report.title)}</h2><p>${escapeHtml(report.summary)}</p><span class="report-link">Bericht ansehen →</span></div></a></article>`).join("") : `<div class="reports-empty"><h2>Die ersten Berichte werden vorbereitet.</h2><p>Hier erscheinen demnächst dokumentierte Reparaturen mit Fotos und verständlichen Erklärungen.</p></div>`;
