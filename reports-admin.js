(() => {
  ...
  function syncImageItems(items) {
    ...
    clearPreviewUrls();
    state.previewUrls = state.imageItems.map((item) => item.file ? URL.createObjectURL(item.file) : `/api/reparaturberichte/bild/${encodeURIComponent(item.id)}?v=20260914-2`);
